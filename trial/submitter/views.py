...
def submitter_landing_view(request, *args, **kwargs):
	...
	return render(request, "submitter_landing.html", {"submitter": submitter, "approved_tasks": approved_tasks, "pending_tasks": pending_tasks,})
...
def submitter_file_upload(request, pk):
	...
	return render(request, "upload.html", {"data_types": data_types, "form": form, "task":task, "uploaded_files": uploaded_files})
...
def submitted(request, pk):
	...
	submitted = RawDataSeqFile.objects.create(submitter=submitter, file=file, raw_data_type=raw_data_type, round=round, term_start=term_start, term_end=term_end)
	submitted.save()
