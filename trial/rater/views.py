...
def assigned_landing_view(request, *args, **kwargs):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            rated = ParsedData.objects.create(rater=rater, raw_data_seq_file=raw_data_seq_file, submitter=submitter, task=task,
                                                      total_tuple_num=total_tuple_num, duplicate_tuple_num=duplicate_tuple_num, column_null_ratio=column_null_ratio, quantity_score=quantity_score, quality_score=quality_score, evaluated=evaluated, pass_or_not=pass_or_not)
            rated.save()
